...
def _make_username(base):
    ...
    suffix = 0
    while True:
        cursor.execute("SELECT user_id FROM user WHERE username=?", (candidate,))
...
def google_callback():
    ...
    if user:
        ...
        if not user["google_id"]:
            cursor.execute("UPDATE user SET google_id=? WHERE user_id=?", (google_id, user["user_id"]))
            conn.commit()
        conn.close()
    ...
    session["role"] = "user"
    # New users get a chance to pick skills before landing on the feed.
